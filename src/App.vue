    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="ticker" @keydown="copy = false" @keydown.enter="add"
                @input="filterExample($event.target.value)" type="text" name="wallet" id="wallet"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Например DOGE" />
            </div>
            <div class="flex bg-white shadow-md p-1 rounded-md flex-wrap" v-if="example && ticker">
              <span v-for="e in example" :key="e"
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer"
                @click="addByExample(e)">
                {{ e }}
              </span>
            </div>
            <div v-if="copy">
              <p class="text-red-500 text-xl">Монета уже добавлена</p>
            </div>
          </div>
        </div>
      ...
      <template v-if="tickers.length">
        ...
        <div>
          ...
          <button @click="page = parseInt(page) + 1" v-if="hasNextPage"
            class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
            Вперед
          </button>
...
export default {
  ...
  methods: {
    ...
    filterExample(p) {
      const names = Object.keys(toRaw(this.namesData));
      let query = p;
      ...
      let a = filterItems(names, query).slice(0, 4);
      this.example = [...a];
    },
  ...
  created() {
    fetch(
      "https://min-api.cryptocompare.com/data/all/coinlist?summary=true&api_key=b28f4d17ee44cf0a57d731304041322c33d63e8741d1652f9fc6615859a67f43"
    ).then((response) =>
      response.json().then((data) => {
        this.namesData = data.Data;
      })
    );
